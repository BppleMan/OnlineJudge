...
function addOneSelectProblem(problemId, problemTitle) {
    ...
    addToProblemList(problemId, problemTitle);
}
...
function removeOneSelectProblem(problemId) {
    ...
    removeFromProblemList(problemId);
}
...
// 添加一个li到sidebarList
function addToProblemList(problemId, problemTitle) {
    var ul = $("#problemList");
    var li = $("<li value='"+problemId+"'></li>");
    var a = $("<a href='#' data-toggle='tooltip' data-placement='bottom' title='点击删除' onclick=removeOneSelectProblem(" + problemId + ")></a>").text(selectedProblemIds.length + ":" + problemTitle);
    li.append(a);
    ul.append(li);
...
// 删除一个li从sidebarList
function removeFromProblemList(problemId) {
    var li = sidebarLi[problemId];
    $(li).remove();
    delete sidebarLi[problemId];
}

function emptyProblemList() {
...
function createFormAndSubmit() {
    var action = basePath + "/bg/contest/edit_contest/content/submit";
...
function generateProblem() {
    ...
    if (canGenerate) {
        $.ajax({
            url : basePath + "/smartProblem/generateProblemByAdmin",
            type : "POST",
            dataType : "JSON",
